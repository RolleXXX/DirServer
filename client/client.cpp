...
Client::Client(char* ip,int pt):serverip(ip),port(pt){

}

void Client::setPath(char* path){
    this->path = path;
}

void Client::setServerpath(char* server_path){
    this->server_path=server_path;
}

void Client::setLocalpath(char* localpath){
    this->localpath = localpath;
}


int Client::upload_file_to_server(){
...
int Client::connect_server(char *serverip, int port){
...
int Client::check_warp(int len,char* addr){
...
int Client::walkdir(char *path, char *files)
{
    DIR *dir;
    struct dirent *ptr;
    int len = 0;
...
int Client::sendfile(int confd,char* root_path, char *file, int if_fname_fin)
{
    int content_len;    
...
void Client::sendreg(int confd,char* file,int if_fname_fin)
{
    int content_len;
    char send_buf[BUF_SZ] = {0};
    FILE *fp = fopen(file, "rb");
...
int Client::save_file_from_server(){
    ...
    // send path
    send_len = strlen(server_path) + 1;
    strncpy(send_buf, server_path, strlen(server_path) + 1);
...
int Client::check_mkpath(char *dirpath){
    char dir[1024] = {0};
...
int Client::recvfile(int sockfd, char *server_root, char *localpath, int *final_flg){
...
int main(){
    ...
    client.setServerpath(server_path);
    client.save_file_from_server();
